...
${i === 0 ? `We will start by playing a fun rhyming game! You will learn to spot words that sound alike at the end.

✏️ Activities:
- Find rhyming pairs in flashcard games
- Sort words by their beginning sounds
- Work in groups to make word lists
` : `Building on what we learned, we'll practice more with ${mainStrand?.subStrand || 'our topic'}.
...
const PlannerPage: React.FC = () => {
  ...
  const { plans: savedLessonPlans, loading: plansLoading, savePlan, updatePlan, deletePlan } = useLessonPlans();
  ...
  const handleRegenerate = async () => {
    ...
    setGeneratedPlan({ ...generatedPlan, id: crypto.randomUUID(), createdAt: new Date() });
    setIsGenerating(false);
    toast({ title: "Plan Regenerated", description: "A new version of your lesson plan is ready." });
  };

  const handleSave = async (plan: GeneratedLessonPlan) => {
    // Check if this plan already exists in DB (is being edited from history)
    const existingPlan = savedLessonPlans.find(p => p.id === plan.id);
    if (existingPlan) {
      await updatePlan(plan.id, plan);
      toast({ title: "Lesson Plan Updated!", description: "Your changes have been saved." });
    } else {
      await savePlan(plan);
      toast({ title: "Lesson Plan Saved!", description: "Your lesson plan has been saved." });
    }
      ...
      <div className="container mx-auto px-4 lg:px-8 py-8 max-w-5xl">
        <div className="mb-8">
          <div className="flex items-center justify-between mb-2">
            <h1 className="text-2xl lg:text-3xl font-bold text-foreground">Lesson Plan</h1>
            <Button onClick={handleNewPlan} variant="outline"><Plus className="w-4 h-4 mr-2" /> New Plan</Button>
          </div>
        </div>
        <LessonPlanDisplay plan={generatedPlan} onRegenerate={handleRegenerate} onSave={handleSave} onEdit={handleEdit} isRegenerating={isGenerating} />
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 lg:px-8 py-8 max-w-5xl">
      <div className="mb-8">
        <h1 className="text-2xl lg:text-3xl font-bold text-foreground mb-2">Lesson Planner</h1>
        <p className="text-muted-foreground">Generate GES-aligned lesson plans powered by AI</p>
      </div>

      <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as 'create' | 'history')}>
        <TabsList className="mb-6">
          <TabsTrigger value="create" className="flex items-center gap-2"><Sparkles className="w-4 h-4" /> New Plan</TabsTrigger>
          <TabsTrigger value="history" className="flex items-center gap-2">
            <History className="w-4 h-4" /> History
            {savedLessonPlans.length > 0 && (
              <span className="ml-1 px-2 py-0.5 text-xs bg-primary/10 text-primary rounded-full">{savedLessonPlans.length}</span>
            )}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="history" className="mt-0">
          {plansLoading ? (
            ...
            <PlanHistoryList plans={savedLessonPlans} onView={handleViewPlan} onDelete={handleDeletePlan} />
          )}
        </TabsContent>
      </Tabs>
    </div>
