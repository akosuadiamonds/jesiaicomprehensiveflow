          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5">
                {topTeachers.map((t, i) => (
                  <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-accent/5 border border-accent/15 hover:bg-accent/10 transition-colors">
                    <div className="w-9 h-9 rounded-full bg-accent/20 flex items-center justify-center shrink-0 text-sm font-bold text-accent-foreground">
                      {t.name.split(' ').map(n => n[0]).join('')}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-sm font-medium text-foreground truncate">{t.name}</p>
                        <Badge variant="secondary" className="text-[10px] bg-accent/15 text-accent-foreground shrink-0">{t.note}</Badge>
                      </div>
                      <p className="text-xs text-muted-foreground mt-0.5">{t.subjects?.join(', ') || 'N/A'} · {t.classGrade || 'N/A'}</p>
                      <p className="text-xs text-accent-foreground/70 mt-1 flex items-start gap-1">
